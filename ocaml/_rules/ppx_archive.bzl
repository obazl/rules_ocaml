...
def _ppx_archive_impl(ctx):
  ...
  # result        = compile_archive("ppx_archive", ctx, mode, mydeps)
  ...
  # options = get_options("ppx_archive", ctx)
  ...
  #         cmxa = result.cmxa, ## obj["cmxa"] if "cmxa" in obj else None,
  #         a    = result.a     ## obj["a"] if "a" in obj else None
  #     )
  #     directs = [result.cmxa, result.a]
  # else:
  #     payload = OcamlArchivePayload(
  #         archive = ctx.label.name,
  #         cma     = result.cma,  ## obj["cm_a"] if "cm_a" in obj else None,
  #     )
  #     directs = [result.cma]
...
ppx_archive = rule(
    implementation = impl_archive, # _ppx_archive_impl,
