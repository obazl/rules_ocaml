...
################
ppx_ns_env = rule(
  implementation = _impl_ppx_ns_env,
...
You may use the [ppx_ns_env](macros.md#ppx_ns_env) macro instead of instantiating this rule directly.
...
instance can use the prefix string of an `ppx_ns_env` to rename its
source file by using attribute `ns` to reference the label of an
`ppx_ns_env` target. Instances of `ocaml_ns_module` can list such
modules as `submodule` dependencies. They can also use an
`ppx_ns_env` prefix string to name themselves, by using their `ns`
...
ns(ns = "foobar", srcs = glob(["*.ml"]))
    ...
    attrs = dict(
        ...
        _rule = attr.string(default = "ppx_ns_env")
    ),
