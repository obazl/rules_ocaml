...
load("//ocaml/_rules:impl_ns.bzl", "impl_ns")
...
You may use the [ns](macros.md#ns) macro instead of instantiating this rule directly.
...
instance can use the prefix string of an `ocaml_ns` to rename its
source file by using attribute `ns` to reference the label of an
`ocaml_ns` target. Instances of `ocaml_ns_module` can list such
modules as `submodule` dependencies. They can also use an
`ocaml_ns` prefix string to name themselves, by using their `ns`
...
ns(ns = "foobar", srcs = glob(["*.ml"]))
    ...
    attrs = dict(
        ...
        resolver   = attr.label(
            doc = "Experimental",
            default = "@ocaml//ns:resolver"
        ),
        submodules = attr.label( # _list(
            default = "@ocaml//ns:submodules",
