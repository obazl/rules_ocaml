...
def impl_ns_resolver(ctx):
    ...
    if len(submodules) < 1:
        if debug:
            print("NO SUBMODULES: returning null ns")
        ...
        for submodule in submodules:  # e.g. [Color, Red, Green, Blue], where main = Color
            # submodule = normalize_module_name(submodule_label)
            if submodule == ns_prefix:  # submodule is 'main'
                resolver_module_name = resolver_module_name + "__0Resolver"
    ...
    nsProvider = OcamlNsResolverProvider(
        ...
        prefix   = ns_prefix,  ## aliasing prefix
