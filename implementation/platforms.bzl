...
def _generate_constraints(names, bazel_constraints):
    return {
        name: bazel_constraints.get(name, "@obazl_rules_ocaml//toolchain:" + name)
        for name in names
    }
...
def _generate_platforms():
    ...
    for ocamlos, ocamlarch in OCAMLOS_OCAMLARCH:
        ...
        platforms.append(struct(
            ...
            ocamlos = ocamlos,
            ocamlarch = ocamlarch,
            constraints = constraints + ["@obazl_rules_ocaml//toolchain:cocaml_off"],
            cocaml = False,
        ))
        if (ocamlos, ocamlarch) in COCAML_OCAMLOS_OCAMLARCH:
            ...
            platforms.append(struct(
                ...
                ocamlos = ocamlos,
                ocamlarch = ocamlarch,
                constraints = constraints + ["@obazl_rules_ocaml//toolchain:cocaml_on"] + mingw,
                cocaml = True,
            ))
    ...
    for ocamlarch in ("arm", "arm64", "386", "amd64"):
        ...
        platforms.append(struct(
            ...
            ocamlos = "darwin",
            ocamlarch = ocamlarch,
            constraints = constraints + ["@obazl_rules_ocaml//toolchain:cocaml_on"],
            cocaml = True,
        ))
